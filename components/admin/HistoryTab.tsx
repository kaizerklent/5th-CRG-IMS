    ...
    <div className="fixed inset-0 bg-black/85 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="relative w-full max-w-2xl flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="bg-red-600 text-white px-4 py-3 rounded-t-xl flex items-center gap-3">
          ...
          {urls.length > 1 && <span className="text-xs text-red-200 font-medium flex-shrink-0">{idx + 1} / {urls.length}</span>}
          <button onClick={onClose} className="w-7 h-7 bg-red-700 hover:bg-red-800 rounded-lg flex items-center justify-center transition flex-shrink-0">
      ...
      <div className="flex flex-col items-start gap-1.5" style={{ minWidth: 80 }}>
        <div className="relative group w-16 h-16 flex-shrink-0">
          <button onClick={() => setLightboxOpen(true)}
            className="w-full h-full block rounded-xl overflow-hidden ring-2 ring-red-200 hover:ring-red-400 transition focus:outline-none" title="Click to view full size">
          ...
          {photos.length > 1 && (
            <>
              <button onClick={prev} className="absolute left-0.5 top-1/2 -translate-y-1/2 w-5 h-5 bg-black/60 hover:bg-black/80 rounded-full flex items-center justify-center transition opacity-0 group-hover:opacity-100 z-10" title="Previous photo">
              ...
              <button onClick={next} className="absolute right-0.5 top-1/2 -translate-y-1/2 w-5 h-5 bg-black/60 hover:bg-black/80 rounded-full flex items-center justify-center transition opacity-0 group-hover:opacity-100 z-10" title="Next photo">
...
function InventoryActivityLog() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, snap => {
      const docs = snap.docs.map(d => ({ id: d.id, ...d.data() } as AdminHistory));
      setLogs(docs);
      lastDocRef.current = snap.docs[snap.docs.length - 1] ?? null;
      setHasMore(snap.docs.length === HISTORY_BATCH);
      setLoading(false);
    });
    return () => unsub();
  }, []);
  ...
  async function loadMore() {
    ...
    try {
      ...
      const more = snap.docs.map(d => ({ id: d.id, ...d.data() } as AdminHistory));
      ...
      <div className="card px-5 py-4">
        <div className="flex flex-wrap gap-3 items-end">
          <div className="flex-1 min-w-[180px]">
            ...
            <input type="text" placeholder="Item name, admin, details..."
          ...
          {hasFilters && <button onClick={clear} className="btn-secondary">Clear Filters</button>}
        </div>
      </div>
                ...
                >
                  {loadingMore
                    ...
                    : <>
                        ...
                        Load {HISTORY_BATCH} more records
                      </>
                  }
                </button>
              )}
              {!hasMore && logs.length > HISTORY_BATCH && (
                <span className="text-xs text-gray-400 italic">All records loaded ({logs.length} total)</span>
              )}
            </div>
            {totalPages > 1 && (
    ...
    <div className="space-y-4">
      {damagedCount > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-xl px-5 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-red-800">{damagedCount} item{damagedCount !== 1 ? 's' : ''} returned damaged</p>
              <p className="text-xs text-red-600 mt-0.5">These items are marked Unavailable in inventory until reviewed.</p>
            </div>
          </div>
          <button onClick={() => { setCondFilter('Damaged'); setStatus('Returned'); setPage(1); }}
      ...
      <div className="card px-5 py-4">
        <div className="flex flex-wrap gap-3 items-end">
          <div className="flex-1 min-w-[180px]">
            <label className="block text-xs font-medium text-gray-600 mb-1">Search</label>
            <input type="text" placeholder="Borrower name or item name..." value={search}
              onChange={e => { setSearch(e.target.value); setPage(1); }} className="input-base"/>
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Status</label>
            <select value={status} onChange={e => { setStatus(e.target.value as any); setPage(1); }} className="input-base bg-white w-auto">
          ...
          <div>
            ...
            <select value={condFilter} onChange={e => { setCondFilter(e.target.value as any); setPage(1); }} className="input-base bg-white w-auto">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={r.id} className={`border-b border-gray-50 hover:bg-gray-50 transition ${r.returnCondition === 'Damaged' ? 'bg-red-50/40' : ''}`}>
                  ...
                  <td className="px-5 py-4"><span className={r.status === 'Returned' ? 'badge-returned' : 'badge-approved'}>{r.status}</span></td>
                  <td className="px-5 py-4">{condBadge(r.returnCondition)}</td>
                  <td className="px-5 py-4"><DamagePhotosCarousel req={r}/></td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  return (
    <div className="w-full space-y-4">
      <div className="flex gap-1 bg-gray-100 rounded-xl p-1 w-fit">
        <button onClick={() => setSubTab('borrows')}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition ${subTab === 'borrows' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
          📋 Borrow Logbook
        </button>
        <button onClick={() => setSubTab('inventory')}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition ${subTab === 'inventory' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
