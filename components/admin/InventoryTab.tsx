  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {[
          ...
          <div key={label} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <span style={{ color: 'rgba(255,255,255,0.78)', fontSize: 9.5, textTransform: 'uppercase', letterSpacing: '0.7px', width: 108, flexShrink: 0 }}>
              {label}
            </span>
            <div style={{ flex: 1, background: 'white', borderRadius: 3, padding: '3px 8px', minHeight: 22 }}>
              <span style={{ color: value ? '#1a3870' : '#aab4c8', fontSize: 11, fontWeight: 600, fontFamily: label === 'Inventory Nr.:' ? 'monospace' : 'Arial, sans-serif' }}>
                {value || '—'}
              </span>
            </div>
          </div>
      ...
      <div style={{ marginTop: 10, borderTop: '1px solid rgba(255,255,255,0.2)', paddingTop: 7 }}>
        <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 7.5, textAlign: 'center', textTransform: 'uppercase', letterSpacing: '0.9px', margin: 0 }}>
                ...
                <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      {/* Thumbnail — clickable to open lightbox if image exists */}
                      <div
                        className={`w-9 h-9 rounded-lg bg-gray-100 flex items-center justify-center flex-shrink-0 overflow-hidden ${item.imageUrl ? 'cursor-pointer hover:opacity-80 transition' : ''}`}
                        onClick={() => item.imageUrl && setLightboxUrl(item.imageUrl)}
                        title={item.imageUrl ? 'Click to enlarge' : undefined}
                      >
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
            ...
            <div className="overflow-y-auto flex-1 px-6 py-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="flex gap-2" role="group" aria-label="Asset type">
                    {[['Unique Asset', true], ['Bulk Item', false]].map(([label, val]) => (
                      <button key={String(val)} type="button"
                        ...
                        aria-pressed={form.isUnique === val}
                ...
                {/* ── Item Image — Cloudinary uploader ── */}
                <div className="col-span-2">
                  <ImageUploader
                    folder="inventory"
                    currentUrl={form.imageUrl}
                    onUploaded={url => upd('imageUrl', url)}
                    onRemove={() => upd('imageUrl', null)}
                    label="Item Image"
                    optional={true}
                    hint="Upload a clear photo of the item. JPEG, PNG, or WebP — max 10 MB."
                  />
                </div>
                ...
                <div className="col-span-2">
                  <div className="border border-gray-200 rounded-xl overflow-hidden">
                    <button type="button" onClick={() => setShowSticker(p => !p)}
                      ...
                      <svg className={`w-4 h-4 text-gray-400 transition-transform ${showSticker ? 'rotate-180' : ''}`}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4"
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col">
            ...
            <div className="overflow-y-auto flex-1 px-6 py-5 space-y-4">
              ...
              <div
                className={`w-full h-44 bg-gray-100 rounded-xl flex items-center justify-center overflow-hidden ${selItem.imageUrl ? 'cursor-pointer hover:opacity-90 transition' : ''}`}
                onClick={() => selItem.imageUrl && setLightboxUrl(selItem.imageUrl)}
                title={selItem.imageUrl ? 'Click to enlarge' : undefined}
              >
                {selItem.imageUrl
      ...
      {lightboxUrl && (
        ...
        >
          <div className="relative max-w-3xl w-full" onClick={e => e.stopPropagation()}>
            <img src={lightboxUrl} alt="Full size preview" className="w-full max-h-[80vh] object-contain rounded-xl shadow-2xl"/>
            <button
              onClick={() => setLightboxUrl(null)}
